...
import { bySlug, byCategory } from '@/lib/products'

type PageContext = {
  pageType: 'home' | 'category' | 'product' | 'other'
  category?: string
  product?: { sku: string; name: string; price: number; category: string; slug: string }
}
...
function getCtx(pathname: string): PageContext {
  if (pathname.startsWith('/producto/')) {
    ...
    const p = bySlug(slug)
    return p
      ? { pageType: 'product', category: p.category, product: { ...p, slug } }
      : { pageType: 'product' }
  }
